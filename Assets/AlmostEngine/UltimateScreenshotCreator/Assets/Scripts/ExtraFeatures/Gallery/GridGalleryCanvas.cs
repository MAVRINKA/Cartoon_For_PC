...
namespace AlmostEngine.Screenshot.Extra
{
	...
	public class GridGalleryCanvas : ScreenshotGallery
	{
        ...
        public virtual void RemoveAllImage()
        {
            for (int i = 0; i < m_ImageFiles.Count; i++)
            {
                File.Delete(m_ImageFiles[i].m_Fullname);
            }
        ...
        {
            StartCoroutine(UpdatePhotosGallery());
        }

        public IEnumerator UpdatePhotosGallery()
        {
